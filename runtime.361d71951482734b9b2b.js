(() => {
  "use strict";
  var e,
    v = {},
    m = {};
  function r(e) {
    var n = m[e];
    if (void 0 !== n) return n.exports;
    var t = (m[e] = { exports: {} });
    return v[e](t, t.exports, r), t.exports;
  }
  (r.m = v),
    (e = []),
    (r.O = (n, t, f, o) => {
      if (!t) {
        var a = 1 / 0;
        for (i = 0; i < e.length; i++) {
          for (var [t, f, o] = e[i], s = !0, u = 0; u < t.length; u++)
            (!1 & o || a >= o) && Object.keys(r.O).every((p) => r.O[p](t[u]))
              ? t.splice(u--, 1)
              : ((s = !1), o < a && (a = o));
          if (s) {
            e.splice(i--, 1);
            var l = f();
            void 0 !== l && (n = l);
          }
        }
        return n;
      }
      o = o || 0;
      for (var i = e.length; i > 0 && e[i - 1][2] > o; i--) e[i] = e[i - 1];
      e[i] = [t, f, o];
    }),
    (r.n = (e) => {
      var n = e && e.__esModule ? () => e.default : () => e;
      return r.d(n, { a: n }), n;
    }),
    (r.d = (e, n) => {
      for (var t in n)
        r.o(n, t) &&
          !r.o(e, t) &&
          Object.defineProperty(e, t, { enumerable: !0, get: n[t] });
    }),
    (r.f = {}),
    (r.e = (e) =>
      Promise.all(Object.keys(r.f).reduce((n, t) => (r.f[t](e, n), n), []))),
    (r.u = (e) =>
      e +
      "." +
      {
        49: "10f88e91b28cc929e5e7",
        148: "43202c2c21b0ebd641bc",
        261: "c44704a16bf30a35aa81",
        523: "41843b110a5e5b8f761c",
        708: "92fedd7db57d72ec451f",
        891: "320a09fa70522ab751f5",
        989: "113eb6b6d9c8ab11e768",
        997: "8f7c1320c2f508b07a3c",
      }[e] +
      ".js"),
    (r.miniCssF = (e) => "styles.d8bacec0301d0935cd78.css"),
    (r.o = (e, n) => Object.prototype.hasOwnProperty.call(e, n)),
    (() => {
      var e = {},
        n = "countries-comparison:";
      r.l = (t, f, o, i) => {
        if (e[t]) e[t].push(f);
        else {
          var a, s;
          if (void 0 !== o)
            for (
              var u = document.getElementsByTagName("script"), l = 0;
              l < u.length;
              l++
            ) {
              var c = u[l];
              if (
                c.getAttribute("src") == t ||
                c.getAttribute("data-webpack") == n + o
              ) {
                a = c;
                break;
              }
            }
          a ||
            ((s = !0),
            ((a = document.createElement("script")).charset = "utf-8"),
            (a.timeout = 120),
            r.nc && a.setAttribute("nonce", r.nc),
            a.setAttribute("data-webpack", n + o),
            (a.src = r.tu(t))),
            (e[t] = [f]);
          var d = (g, p) => {
              (a.onerror = a.onload = null), clearTimeout(b);
              var _ = e[t];
              if (
                (delete e[t],
                a.parentNode && a.parentNode.removeChild(a),
                _ && _.forEach((h) => h(p)),
                g)
              )
                return g(p);
            },
            b = setTimeout(
              d.bind(null, void 0, { type: "timeout", target: a }),
              12e4
            );
          (a.onerror = d.bind(null, a.onerror)),
            (a.onload = d.bind(null, a.onload)),
            s && document.head.appendChild(a);
        }
      };
    })(),
    (r.r = (e) => {
      "undefined" != typeof Symbol &&
        Symbol.toStringTag &&
        Object.defineProperty(e, Symbol.toStringTag, { value: "Module" }),
        Object.defineProperty(e, "__esModule", { value: !0 });
    }),
    (() => {
      var e;
      r.tu = (n) => (
        void 0 === e &&
          ((e = { createScriptURL: (t) => t }),
          "undefined" != typeof trustedTypes &&
            trustedTypes.createPolicy &&
            (e = trustedTypes.createPolicy("angular#bundler", e))),
        e.createScriptURL(n)
      );
    })(),
    (r.p = ""),
    (() => {
      var e = { 666: 0 };
      (r.f.j = (f, o) => {
        var i = r.o(e, f) ? e[f] : void 0;
        if (0 !== i)
          if (i) o.push(i[2]);
          else if (666 != f) {
            var a = new Promise((c, d) => (i = e[f] = [c, d]));
            o.push((i[2] = a));
            var s = r.p + r.u(f),
              u = new Error();
            r.l(
              s,
              (c) => {
                if (r.o(e, f) && (0 !== (i = e[f]) && (e[f] = void 0), i)) {
                  var d = c && ("load" === c.type ? "missing" : c.type),
                    b = c && c.target && c.target.src;
                  (u.message =
                    "Loading chunk " + f + " failed.\n(" + d + ": " + b + ")"),
                    (u.name = "ChunkLoadError"),
                    (u.type = d),
                    (u.request = b),
                    i[1](u);
                }
              },
              "chunk-" + f,
              f
            );
          } else e[f] = 0;
      }),
        (r.O.j = (f) => 0 === e[f]);
      var n = (f, o) => {
          var u,
            l,
            [i, a, s] = o,
            c = 0;
          for (u in a) r.o(a, u) && (r.m[u] = a[u]);
          if (s) var d = s(r);
          for (f && f(o); c < i.length; c++)
            r.o(e, (l = i[c])) && e[l] && e[l][0](), (e[i[c]] = 0);
          return r.O(d);
        },
        t = (self.webpackChunkcountries_comparison =
          self.webpackChunkcountries_comparison || []);
      t.forEach(n.bind(null, 0)), (t.push = n.bind(null, t.push.bind(t)));
    })();
})();
